...
Slice 2: staff can activate/deactivate a template. Follows admin/orders.py's
status-update route exactly — fetch-existing (for the audit "before"
snapshot) -> update -> `record_event` -> return the refreshed row — the
same "service does the work, route logs the event right after" pattern,
...
@router.patch("/templates/{template_id}", response_model=CakeTemplateResponse)
def update_template(
    template_id: uuid.UUID,
    ...
    try:
        updated = template_service.update_template(template_id_str, changed_fields)
    ...
    record_event(
        ...
        entity_type="cake_templates",
        entity_id=template_id_str,
        before={key: existing[key] for key in changed_fields},
        after={key: updated[key] for key in changed_fields},
    )
