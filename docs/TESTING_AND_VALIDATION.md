...
## 2. Automated Validation — 579/579 Passing
...
| `test_agent_order_assistant` | 72 | Chat-assisted ordering: catalog id validation, the size-regression fix, the triple order-confirmation gate, the deterministic food-allergy safety gate, WhatsApp/Chat parity, optional pickup date/time capture, and Servings + Event Pricing's own deterministic guest-count gate (76+ never reaches Claude/order creation, a restated guest count is always authoritative over a same-message named size) |
...
| `test_bakery_manager_service` | 34 | AI Bakery Manager — Preview is read-only, an unknown/unrecognized Claude-proposed action type is forced unsafe, missing/too-far-out `pickup_date` correctly stays non-executable, `ready`/`completed` proposals are always recommendation-only, Claude's own opinion is never trusted (only ever downgraded), Execute re-validates every action fresh, a duplicate Execute click stays safe, one failed action doesn't stop the batch, Execute makes zero Claude calls, the bounded planning-prompt context, the shared `priority_service` integration, and guest-count/serving-band shown as evidence only (never authorization) |
| `test_order_service` | 22 | `create_order()`'s own id-validation/pricing/payload-building logic, the shared pickup-datetime validation (past/Monday/hours) and rush-warning note helpers, and Servings + Event Pricing's guest-count-derived size/price for every standard band (guest_count always overrides a mismatched `cake_size_id`; 76+ is rejected, never priced) |
...
| `test_serving_band_service` | 18 | Servings + Event Pricing: every guest-count boundary (8→SMALL through 75→EVENT, 76→CUSTOM_EVENT), invalid/negative/zero input rejection, the one authoritative `is_standard_ordering_eligible()` gate |
...
- **Knowledge base re-ingestion (Servings + Event Pricing)**: re-run again live against production after extending `pricing_policy.md`/`ordering_process.md` (90 → 91 chunks). Verified with 3 real questions: "What size cake should I use for 60 guests?", "What happens for an event with 100 guests?", and "Can customers order online for more than 75 people?" — all three answered correctly and grounded in the new content, both before and after the production deploy.
- **Frontend JS logic (Pickup Date + Order Priority regression fix; Servings + Event Pricing)**: this project's frontend has no test framework — `frontend/tests/*.js` are small, dependency-free Node scripts (via the `vm` module) that run the actual shipped JS files against a minimal fake DOM/sandbox, not a reimplementation. Run with plain `node frontend/tests/<file>.js`.
