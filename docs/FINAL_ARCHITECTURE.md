...
**Status: CURRENT / AUTHORITATIVE.** This document describes the system as it is actually implemented and deployed today, verified against the current repository, live production data, and the current automated test suite (579/579 passing — see `docs/TESTING_AND_VALIDATION.md`) — not against earlier plans or superseded roadmap documents. For the project's development history, see the sprint documents listed in `docs/README.md`.
