...
**Status: CURRENT / AUTHORITATIVE.** This document describes the system as it is actually implemented and deployed today, verified against the current repository, live production data, and the current automated test suite (470/470 passing) — not against earlier plans or superseded roadmap documents. For the project's development history, see the sprint documents listed in `docs/README.md`.
...
UI: one additional card on `admin-dashboard.html`, alongside the existing AI Daily Briefing/Ask Agent/RAG cards — none of them altered.
